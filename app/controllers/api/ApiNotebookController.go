...
func (c ApiNotebook) fixNotebook(notebook *info.Notebook) info.ApiNotebook {
	...
	// NumberNotes was added for the versioned API2 contract. Keep the legacy
	// /api response shape unchanged by only populating it for API2 requests;
	// the omitempty JSON tag then suppresses the field for /api clients.
	numberNotes := 0
	if c.Request != nil && strings.HasPrefix(c.Request.URL.Path, "/api2/") {
		numberNotes = notebook.NumberNotes
	}
	return info.ApiNotebook{
