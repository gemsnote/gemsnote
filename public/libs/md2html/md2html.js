 ...
 *
	...
	<script>
		...
		md2Html('hello, **gemsnote**', document.getElementById('content2'), function allRendered(html) {
			alert(html);
		});
	</script>
 * 
...
(function() {
	...
	function initUml() {
	    ...
	    umlDiagrams.convert = function(target, callback) {
	        ...
	        if(sequenceElems.length > 0 || flowElems.length > 0) {
	        	if(!_loadUmlJs) {
				loadJs('http://gemsnote.com/public/libs/md2html/uml.js', function() {
		        		_loadUmlJs = true;
		                convert();
		            }); 
	        	} else {
	...
	window.md2Html = function(mdText, toElem, callback) {
		...
		// 加载mathJax
		if(mdText.indexOf('$') !== -1) {
			// loadJs("http://gemsnote.com/public/libs/MathJax/MathJax.js?config=TeX-AMS_HTML", function() {
