...
define("tinymce/pasteplugin/Clipboard", [
	...
	return function(editor) {
		...
		// 当url改变时, 得到图片的大小 copy from gemsnote_image
			...
			// use event.originalEvent.clipboard for newer chrome versions
			  ...
			  if (blob) {
			    ...
			    reader.onload = function(event) {
					...
					var d = {};						
					d.id = '__mcenew';
					d.src = "http://gemsnote.com/images/loading-24.gif"; // 写死了
	...
	PluginManager.add('paste', function(editor) {
		...
		function togglePasteCopyImage() {
			if (clipboard.copyImage) {
				...
				if (!userIsInformed2) {
					editor.windowManager.alert(
						"When copy other site's images (not in gemsnote) into editor, it will copy the image into your album."
					);
